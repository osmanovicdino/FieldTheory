...
NCGasR::NCGasR(double ll, int totalN) {
	obj = new LangevinNVT;
	...
	matrix<double> res(totalN,2); // orientation vector
...
//	f.add_interaction_all(nswca);
	...
	double xi = sqrt(24*angd*dt);

	// double exttheta = 0.0;
	// double extphi = 0.0;
	// double ext_strength = 1.0;
	double fac = 1.0;
	// int sweeps = 5;
	// double max_step = 0.2;
...
// void NCGasR::angularforces() {

// }

//Position forces due to position is already encompassed in the code

matrix<double> NCGasR::PositionForcesDueToAngles_mips(double v0) {
	int totalN = obj->getN();
	//what kind of position forces are generated due to angular evolution.
	matrix<double> F(totalN,3);
	//one example is motility:
	
	for(int i = 0 ; i < totalN ; i++) { //this would choose a random new orientation as a perturbation of the previous one
...
matrix<double> NCGasR::AngleForcesDueToPosition(matrix<int> &pairs) {

}

matrix<double> NCGasR::AngleForcesOnly(double B,double theta0,double phi0) {

}


void NCGasR::UpdateAngles(matrix<double> &angularforces) {
	int totalN = obj->getN();
	//we have a set of orientations of all of our particles

	// we have three contributions to random update rule for the angle

	//random kicks

	//global aligning field

	//local aligning interactions


	//RandomKicks

	//matrix<double> da(totalN,2);
	...
	for(int i = 0 ; i < totalN ; i++) { //this would choose a random new orientation as a perturbation of the previous one
		...
		double newtheta = atan2(sqrt(SQR(x2)+SQR(y2)),z2);
		double newphi = atan2(y2,x2);

		// da(i,0) = newtheta;
		// da(i,1) = newphi;
		// da(i,0) = newtheta - theta;
		// da(i,1) = newphi - phi;
		(*orient)(i,0) = newtheta + fac*dt*angularforces(i,0);
		(*orient)(i,1) = newtheta + fac*dt*angularforces(i,1);
	}

	// matrix<double> dexternalfield(totalN,2);
	// // double exttheta;
	// // double extphi;
	// // double ext_strength;
	// for(int i  = 0 ; i < totalN) {
	// 	double theta  = rot(i,0);
	// 	double phi = rot(i,1);		

	// 	dexternalfield(i,0) =  -ext_strength*(cos(extphi - phi)*cos(theta)*sin(exttheta)) + ext_strength*cos(exttheta)*sin(theta);
	// 	dexternalfield(i,1) = -ext_strength*(sin(exttheta)*sin(extphi - phi)*sin(theta));
	// }

	// matrix<double> localalignment(totalN,2);



}

void NCGasR::setkT(double kT) {
	obj->setkT(kT);
}

void NCGasR::seteqeps(double eqepss) {
	int totalN = obj->getN();
	eqeps = eqepss;
	double sigma = 1.0;
	WCAPotential nswca(1.0, sigma, eqeps);
