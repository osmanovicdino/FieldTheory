...
IsingPolymer::IsingPolymer(int Nrr, int no_typess, int copp=0) : radialpoints(vector1<double>(Nrr)) , propd(vector1<double>(Nrr)), amat(matrix<double>(Nrr,Nrr)),bmat(matrix<double>(Nrr,Nrr)),bigD(matrix<double>(Nrr,Nrr)),V(matrix<double>(no_typess+copp,no_typess+copp)),fixed_field(matrix<double>(Nrr,no_typess+copp)) {
	cop = copp;
	ds = 1.0;
	stot =  1000.0;
	totpolymers = 1.0;
...
matrix<double> IsingPolymer::radialsolver_copolymer(vector1<double> &field1, vector1<double> &field2, double &norm) { //MAKE SURE SEQUENCE IS SYMMETRIC
	...
	double bprop = (double)totsize - aprop;
	...
	vector1<long double> denA(Nr);
	vector1<long double> denB(Nr);
	for(int i = 0 ; i < totsize ; i++) {
		if((*seq)[i]) {
		for(int j = 0 ; j < Nr ; j++) {		
	...
	long double na = this->integrate(denA);

	long double nb = this->integrate(denB);
	...
	denA*=((long double)(totpolymers*aprop)/na);
	denB*=((long double)(totpolymers*bprop)/nb);
