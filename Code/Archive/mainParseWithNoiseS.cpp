...
int main(int argc, char **argv)
{
    ...
    bool err11, err2, err3, err4;
    matrix<double> rfield1 = importcsv(fi1, T, err11);
    matrix<double> rfield2 = importcsv(fi2, T, err2);
    matrix<double> rfield3 = importcsv(fi3, T, err3);
    matrix<double> rfield4 = importcsv(fi4, T, err4);

    for (int i = 0; i < p.N1; i++)
    {
        for (int j = 0; j < p.N2; j++)
        {
            field1(i, j) = myc(rfield1(i, j));
            field2(i, j) = myc(rfield2(i, j));
            field3(i, j) = myc(rfield3(i, j));
            field4(i, j) = myc(rfield4(i, j));
        }
    }
