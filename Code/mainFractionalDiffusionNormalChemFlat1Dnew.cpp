...
int main(int argc, char **argv)
{
    ...
    cout << "matrices set" << endl;



    double **fields = new double *[nof];
    for(int j = 0 ; j < nof ; j++) {
    fields[j] = (double *)fftw_malloc(p.N1 * sizeof(double));
    }

    // for (int j = 0; j < nof; j++)
    // {
    //     v.push_back(fields[j]);
    // }
    ...
    for (int i = 0; i < runtime; i++)
    {
        ...
        if (i % every == 0 && i > 00000)
        {
            ...
            myfile << a.fields[0][p.N1 - 1] << endl;

            for (int j = 0; j < p.N1 - 1; j++) {
                myfile2 << a.fields[1][j] << ",";
            }
            myfile2 << a.fields[1][p.N1 - 1] << endl;
