...
int main(int argc, char **argv)
{
    ...
    */
    ...
    double A0 = 0.626;
    double bc = 0.384;
    double i0 = bc * rate1 * p0 / (A0 * rate2);
    ...
    CahnHilliardWithCouplingWeightSQRandLinear d1(c00, c11, nu, eps1, eps2, eps2l, 2, 3);
    DiffusiveWeight d2(1.0);
    DiffDiffusiveWeightSQR d3(eps1, 0);
    DiffDiffusiveWeightSQRandLinear d4(eps2, eps2l, 0);
    ...
    for (int i = 0; i < p.N1; i++)
    {
        for (int j = 0; j < p.N2; j++)
        {
            ...
            field2(i, j) = i0 + (0.2 * ((double)rand() / (double)RAND_MAX) - 0.1);
