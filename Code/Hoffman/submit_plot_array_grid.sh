...
cd /u/scratch/d/dinoo/CLGE5/${dirwemake}
export OMP_NUM_THREADS=1
num=`ls res1r*.csv | wc -l`
if test $num -ge 1;
then
    for file1 in res1r*.csv; do
        base=${file1#res1r}
        ...
        if [[ -f "$file1" && -f "$file2" && -f "$file3" && -f "$file4" ]]; then
            python3 ~/FieldTheory/Code/Plotting/PlotDirectoryArray.py "$file1" "$file2" "$file3" "$file4"; 
        else
            echo "files missing"
        fi
    done
