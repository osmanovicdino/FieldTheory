...
filename=~/FieldTheory/Code/Hoffman/paramsA.dat
basedir="AntiInvasion3D"
if [ -e ${filename}   ]; then
   ...
   fil2=`sed -n ${SGE_TASK_ID}p ${filename} | awk '{print $8}'`
...
./angron $c0 $c1 $l $p0 $A0 $dirc $fil1 $fil2 > log
# echo job info on joblog:
