...
template <class T>
matrix<T>& matrix<T>::operator*=(const matrix<T> &a) {
        if (nrows != a.nrows || ncols != a.ncols) {cout << a << endl; error("diff size matrics in *= operator");}
...
sum += m1.mat[i*numcols+j]*m2.mat[j*numcols+k];
}
...
template <class T>
void ft2D(matrix<T> &m, matrix<T> &reout, matrix<T> &imout) {
    int height = m.nrows;
    int width = m.ncols;
        ...
        for (int xWave = 0; xWave < width; xWave++)
        {
            ...
            for (int ySpace = 0; ySpace < height; ySpace++)
            {
                for (int xSpace = 0; xSpace < width; xSpace++)
                {
                    ...
                    reout(xWave, yWave) += (m(xSpace,ySpace) * cos( 2 * pi * ((1.0 * xWave * xSpace / width) + (1.0 * yWave * ySpace / height)))) / sqrt(width * height);
                    imout(xWave, yWave) -= (m(xSpace, ySpace) * sin(2 * pi * ((1.0 * xWave * xSpace / width) + (1.0 * yWave * ySpace / height)))) / sqrt(width * height);
                    //phout(xWave, yWave) = sqrt(realOut[yWave][xWave] * realOut[yWave][xWave] + imagOut[yWave][xWave] * imagOut[yWave][xWave]);
                }
...
int FFT2D(matrix<COMPLEX> &c, int nx, int ny, int dir)
{
    int i, j;
    int m, twopm;
    double *real, *imag;
    ...
    for (j = 0; j < ny; j++)
    {
        for (i = 0; i < nx; i++)
        {
            real[i] = c.mat[i*nx+j].real;
            imag[i] = c.mat[i*nx+j].imag;
        }
        FFT(dir, m, real, imag);
        for (i = 0; i < nx; i++)
        {
            c.mat[i * nx + j].real = real[i];
            c.mat[i * nx + j].imag = imag[i];
        }
    }
    ...
    for (i = 0; i < nx; i++)
    {
        for (j = 0; j < ny; j++)
        {
            real[j] = c.mat[i * nx + j].real;
            imag[j] = c.mat[i * nx + j].imag;
        }
        FFT(dir, m, real, imag);
        for (j = 0; j < ny; j++)
        {
            c.mat[i * nx + j].real = real[j];
            c.mat[i * nx + j].imag = imag[j];
        }
    }
    free(real);
    free(imag);
